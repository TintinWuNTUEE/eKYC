...
class HoloDetector:
    ...
    def checkedTrace(img0, img1, p0, back_threshold=1.0):
        ...
        return p1, status
    async def calc_filtered_points(self,qr,ms,mv):
        filtered_points = []
    ...
    async def calc_holo_points(self):
        ...
        filtered_points = await self.calc_filtered_points(qr,ms,mv)
    ...
    async def lock_rect(self, rect, gray):
    ...
    async def detect_holos(self, img, search_rect=False):
        """
        ...
        if self.is_training:
            ...
            if search_rect:
                ...
                if len(self.rect_areas) > HoloDetector.TRAIN_FRAMES:
                    ...
                    if np.std(self.rect_areas) < 0.005 * np.mean(self.rect_areas):
                        await self.lock_rect(rect, gray)
                    else:
                        self.reset()
            else:
            ...
            if self.stack_filled:
                ...
                filter_points = await self.calc_holo_points()
        ...
        return holo_mask_t, img_holo,filter_points
